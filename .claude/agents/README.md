...
| 2026-05-14 | design | Sales 페이지만 `max-w-5xl` (1024px)·다른 페이지는 `max-w-3xl` (768px). 캐논 개별 항목은 다 통과해도 페이지 간 구성 일관성 없어 "Sales만 모바일을 PC에 늘려놓은" 인상 — 사장님이 "디자인 핍진성 해친다"로 잡음. | `design-reviewer.md` #11 직전에 **Cross-page composition** 룰 추가: 변경 페이지 + 이웃 1-2 페이지 같은 viewport에서 동시 캡처 → max-w·heading 위계·그리드 밀도·여백 리듬·빈상태 구성 항목별 비교, 외톨이 있으면 flag. Sales-specific fix: max-w-5xl→max-w-4xl + xl:grid-cols-3 + 카테고리 라벨 위계 + 빈 cart 중앙 정렬 + 🛒 placeholder. |
